    ...
    .then(() => {
            ...
            try {
                ...
                logic.authenticateUser(email, password)
                    .then(user => {
                        ...
                        const token = jwt.sign({ sub: user.id }, JWT_SECRET, { expiresIn: '120m' })
        ...
        server.put('/events/:eventId', jsonBodyParser, (req, res) => {
            try {
                ...
                logic.editEvent(eventId, { userId, type, title, date, duration, address, description })
                    .then(updatedEvent => res.status(200).json({ message: 'Event edited successfully', updatedEvent }))
                    .catch(error => {
                        let status = 500;
        ...
        // selectedEvent
        server.put('/events/select/:eventId', jsonBodyParser, (req, res) => {
            try {
                const { authorization } = req.headers
                const token = authorization.slice(7)
                const { sub: userId } = jwt.verify(token, JWT_SECRET)
                ...
                logic.selectedEvent( userId, eventId )
                    .then(() => res.status(204).send())
                    .catch(error => {
                        let status = 500
                        if (error instanceof TypeError || error instanceof RangeError || error instanceof ContentError) {
        ...
        // deselectEvent
        server.put('/events/deselect/:eventId', jsonBodyParser, (req, res) => {
            try {
                const { authorization } = req.headers
                const token = authorization.slice(7)
                const { sub: userId } = jwt.verify(token, JWT_SECRET)
                ...
                logic.deselectedEvent(userId, eventId)
                    .then(() => res.status(204).send())
                    .catch(error => {
                        let status = 500
                        if (error instanceof TypeError || error instanceof RangeError || error instanceof ContentError) {
