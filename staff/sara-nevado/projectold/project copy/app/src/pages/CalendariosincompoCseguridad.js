...
function Calendar() {
            ...
            {eventForDay && (
              ...
              >
                ...
                <div id={`subscriber-info-${day}`} className="subscriber-info absolute bottom-0 left-0 p-1 bg-gray-800 text-white text-xs rounded w-full" style={{ display: 'none' }}>
                  Seleccionado por: {eventForDay.subscribers.map(subscriberId => (
                    <span key={subscriberId}>{subscriberId}</span>
                  ))}
                </div>
              </div>
            )}
  ...
  const handleSelectEvent = async (eventId) => {
    try {
      ...
      updateEventSubscribers(eventId, user.id, true);
    } catch (error) {
  ...
  const handleDeselectEvent = async (eventId) => {
    try {
      ...
      updateEventSubscribers(eventId, user.id, false);
    } catch (error) {
      console.error('Error deselecting event:', error);
    }
  };

  const updateEventSubscribers = (eventId, userId, isSelected) => {
    const updatedEvents = events.map(event => 
      event.id === eventId ? {
        ...event, 
        subscribers: isSelected 
          ? [...event.subscribers, userId] 
          : event.subscribers.filter(subscriber => subscriber !== userId)
      } : event
    );
