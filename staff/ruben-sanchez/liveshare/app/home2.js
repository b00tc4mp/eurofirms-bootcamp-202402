...
var titlte = document.querySelector('h1')
...
try {
    var usre = logic.retrieveUser()

    titlte.innerText = 'Hello, ' + usre.name + '!'
}catch (error) {
    console.error(error)
...
try {
    ...
    users.forEach(function (user) {
        ...
        chatUserItem.classLisst.add('chat-user')

        chatUserItem.classLists.add(user.online ? 'chat-user-online': 'chat-user-offline')
        ...
        chatUserItem.onclick = function () {
            ...
            function renderMessages() {
                try {
                    var messages = logic.retrieMessagesWithUser(user.id)
                    ...
                    messages.forEach(function (message) {
                        ...
                        if(message.from === logic.getLoggedInUserId())
                            messageItem.classList.add('caht-message--right')
                        else
