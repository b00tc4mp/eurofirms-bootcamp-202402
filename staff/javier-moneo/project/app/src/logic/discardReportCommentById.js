...
function discardReportCommentById(reportCommentId) {
  validate.token(sessionStorage.token);
  validate.id(reportCommentId);

  //   const { sub: userId } = utils.extractPayload(sessionStorage.token);

  return fetch(`${import.meta.env.VITE_API_URL}/reportComments/discardReport`, {
    method: 'PATCH',
    headers: {
...
export default discardReportCommentById;
