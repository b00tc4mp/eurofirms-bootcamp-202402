...
function discardReportSeachById(reportSearchId) {
  validate.token(sessionStorage.token);
  validate.id(reportSearchId);

  //   const { sub: userId } = utils.extractPayload(sessionStorage.token);

  return fetch(`${import.meta.env.VITE_API_URL}/reportSearches/discardReport`, {
    method: 'PATCH',
    headers: {
...
export default discardReportSeachById;
